...
def main():
    ...
    print("Deriving user_id from doc_id ...")
    df = add_user_id_column(df)

    user_level_labels = None
    if not args.no_stratify and args.label_col in df.columns:
        print("Building user-level labels for stratification (majority label per user) ...")
        user_level_labels = optionally_build_user_labels_for_stratification(df, args.label_col)
    else:
        if args.no_stratify:
            print("Stratification disabled by flag.")
        else:
            print(f"Label column '{args.label_col}' not found. Proceeding without stratification.")

    unique_users = df["user_id"].dropna().unique()
    unique_users = np.array(sorted(unique_users))

    print(
        f"Splitting USERS -> test_size={args.test_size:.3f}, "
    ...
    print("\nDone. No user overlap across splits.")
